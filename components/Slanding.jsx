...
import Sellerstores from './Sstorelist'
...
export default function Sallerland() {
  ...
  return (
    <>
      ...
      >
            ...
            >
              <div className="mr-4 ml-12 flex w-28 max-w-2xl items-center rounded border border-gray-200 bg-white p-2 shadow-sm sm:w-fit  md:w-fit lg:w-fit">
                ...
                <input
                  ...
                  type="search"
                  name="search"
                  placeholder="Search On Your Stores"
              ...
              <ul className="flex items-center ">
                ...
                >
                  <button
                    ...
                    className={
                      Theme
                        ? 'group mx-auto mt-2 flex h-10 items-center justify-center rounded-full bg-gray-50 px-4  font-semibold text-gray-900 transition-colors duration-500 hover:bg-gray-400'
                        : 'group mx-auto mt-2 flex h-10 items-center justify-center rounded-full bg-gray-500  px-4  text-gray-100 shadow-2xl shadow-slate-900 transition-colors duration-500 hover:bg-gray-600 hover:text-gray-50 hover:shadow-sm hover:shadow-slate-300'
                    }
                  >
                    <Icon icon="mdi:store-plus" className="h-8 w-8" />
                    <p className="hidden md:block lg:block">ADD STORE</p>
                  </button>
                </li>
              </ul>
            </div>
        ...
        {!Hover && (
          <>
            <div
              id="storeContainer"
              className="  ml-12 grid w-10/12  grid-cols-1 grid-rows-1 gap-1  px-4 py-4  md:mx-auto md:grid-cols-2 md:gap-8 lg:grid-cols-2 2xl:grid-cols-3"
            >
              <Sellerstores
                search={Search}
                Theme={Theme}
                Storecountesetter={Storecountesetter}
              />
            </div>
          </>
        )}
        ...
        >
          <div className="flex flex-grow flex-col justify-between overflow-y-auto overflow-x-hidden">
            <ul className="flex flex-col space-y-1 py-4">
              ...
              <li className="text-white-600  relative flex h-11 flex-row items-center border-l-4 border-transparent pr-6 focus:outline-none ">
                <span className="ml-3 inline-flex items-center justify-center">
                  <Icon className="h-6 w-6" icon="bx:bx-store-alt" />
                </span>
                <span className="ml-2 truncate text-sm font-semibold tracking-wide">
                  Stores
                </span>
                <span className="ml-auto  block rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium tracking-wide text-green-500">
                  {Storescount}
                </span>
              </li>
              <li className={Hover ? 'block px-5 ' : 'hidden'}>
      ...
      {/* create store form */}
      {Addshow && <Newstore Addshowsetter={Addshowsetter} />}
      {/* create store form */}
